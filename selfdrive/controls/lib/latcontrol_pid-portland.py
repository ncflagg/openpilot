...
class LatControlPID(object):
  ...
  def update(self, active, v_ego, angle_steers, angle_steers_rate, eps_torque, steer_override, CP, VM, path_plan, driver_torque):
    ...
    self.past_data.append([interp_fast(angle_steers - 0.4, self.scales['stock_sensor'], [0, 1]), self.output_steer])  # steer command is already 'normalized'
    ...
    if v_ego < 0.3 or not active:
      ...
      pulse_length = 0.04 #0.05 #0.19  #0.07   #0.1       # length of time (seconds) to max-out to overcome friction
      pulse_window = 0.07 #0.1 #0.38 #0.2-too much? #0.14     # total time in sec before another pulse_length is allowed
      ...
      if len(self.output_steer_history) > 2:
        pulse_predict = \
          (abs(self.output_steer_history[0]) < 0.06 and \
           abs(self.output_steer_history[1]) < 0.06 and \
           abs(self.output_steer_history[2]) < 0.06)
      else:
        pulse_predict = False
