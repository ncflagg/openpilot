...
def learn_angle_model_bias(lateral_control, v_ego, angle_model_bias, c_poly, c_prob, angle_steers, steer_override):
  ...
  # learn less at low speed or when turning                                     # ..should be  "..at HIGH speed.."
  # ..so, it learns MORE on straights
  # Could try:
  #if 0 < v_ego < 30.:
  #  v_ego = 30.
  slow_factor = 1. / (1. + 0.02 * abs(angle_steers) * v_ego)                   # sF gets smaller with greater angles and speed
  #slow_factor = 1. / (1. + 0.02 * 1.5 * v_ego)   # Set 3.7 deg and 70mph (31.) statically
   # 3.7deg was too high
   # 1.5deg maybe marginally better (or no diff)
  alpha_v = alpha * c_prob * (max(v_ego - min_learn_speed, 0.)) * slow_factor   # * sF so, same

  # only learn if lateral control is active and if driver is not overriding:
  if lateral_control and not steer_override:
    angle_model_bias += c_poly[3] * alpha_v                                     # * alpha_v which is * slow_factor, which all get smaller +angle OR +speed
