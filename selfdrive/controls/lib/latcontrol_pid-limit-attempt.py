...
class LatControlPID(object):
  ...
  def update(self, active, v_ego, angle_steers, angle_steers_rate, eps_torque, steer_override, CP, VM, path_plan, driver_torque):
    ...
    if not self.pulsing:
      ...
      max_lim = min(max(prev_output_steer_cmd + STEER_ERROR_MAX, STEER_ERROR_MAX), STEER_MAX)
      min_lim = max(min(prev_output_steer_cmd - STEER_ERROR_MAX, -STEER_ERROR_MAX), -STEER_MAX)
