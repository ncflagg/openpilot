...
class LatControlPID(object):
  ...
  def update(self, active, v_ego, angle_steers, angle_steers_rate, eps_torque, steer_override, CP, VM, path_plan, driver_torque):
    ...
    if v_ego < 0.3 or not active:
      ...
      pulse_height = 0.2 #0.3=450 (w max 1500) # torque value to start with to overcome friction
      pulse_length = 0.07   #0.1       # length of time (seconds) to max-out to overcome friction
      pulse_window = 0.14                # total time in sec before another pulse_length is allowed
